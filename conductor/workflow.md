...
## LLM & Multimodal Evaluation (Evals) Workflow

In addition to standard unit tests, changes to models, judging logic, or strategist prompt synthesis must run through multimodal evals:

1. **Golden Benchmark Sets**: Maintain standardized JSONL datasets containing reference photos, input prompts, generated images, and calibrated human ratings.
2. **Judge Correlation Testing**: Before deploying judge prompt changes, run `bin/calibrate.py` to verify correlation with human ground truth (^2$, mean absolute error).
3. **Regression Testing**: Ensure new prompt strategies or judge rules do not decrease scores on established benchmark sets.
